...
$$
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'transfer_event_outbox_status') THEN
...
CREATE INDEX IF NOT EXISTS ix_transfer_events_pick_pending
    ON transfer_events (created_at) WHERE status = 'PENDING';

CREATE INDEX IF NOT EXISTS ix_transfer_events_retry_failed
    ON transfer_events (next_retry_at, created_at) WHERE status = 'FAILED';
